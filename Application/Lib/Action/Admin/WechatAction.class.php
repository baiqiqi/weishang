			...
			case Wechat::MSGTYPE_EVENT :
				...
				if ($eventype ['event'] == "CLICK") {
					if( $eventype ['key']=='GET_PIC')
					{
						...
						{
							$text = '您还不是族长，不能为您生成推广图片，立即购买成为族长';
					...
					elseif( $eventype ['key']=='GET_URL')
					{
						...
						{
							$text = '您还不是族长，不能为您生成推广链接，立即购买成为族长';
							$weObj->text ( $text )->reply ();
						}
						exit ();
					}
					...
					{
						$text = '恭喜您由公众号：【'.$message_name.'】推荐成为'.$message_name.'的第【'.$result['id'].'】会员！';
					}
					else
					{
						if(!empty($user["l_id"]))
						{
							...
							$text = '恭喜您由【'.$user_info['nickname'].'】推荐成为'.$message_name.'的第【'.$user_id.'】会员.购买马上成为'.$message_name.'的族长，让你每天都能赚大钱，早行动早赚钱！';
						}
						else
						{
